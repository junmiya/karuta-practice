...
ユーザーは、東京大学のスクールカラー「淡青（Tan-sei）」を基調とした、清潔で知的なフラットデザインのUIで学習できる。

**Why this priority**: ユーザー体験とブランドイメージを確立するため。初期の「熊本大学風」から方針変更され、最新の「東大風」デザインが求められている。
...
#### Design Requirements
- **FR-022**: アプリケーションは「東京大学（淡青）」スタイルを採用し、プライマリカラーとして `#0B8BDB` (Tan-sei) を使用しなければならない
...
- **SC-013**: `index.css` および `tailwind.config.js` が、新しいデザイン仕様（UTokyo Style）と整合している

...
### User Story 4 - Design Overhaul (University of Tokyo Style) (Priority: P2)

ユーザーは、スクールカラー「淡青（Tansei Blue）」を基調とした、清潔で知的なフラットデザインのUIで学習できる。

**Why this priority**: ユーザー体験とブランドイメージを確立するため。初期のプロトタイプから方針変更され、最新の「アカデミック・スタイル」デザインが求められている。

**Independent Test**: ブラウザでトップページおよび練習ページにアクセスし、背景やボタンが「淡青（#0B8BDB）」で統一され、グラデーションのないフラットデザインであることを視覚的に確認する。

**Acceptance Scenarios**:

1. **Given** トップページにアクセスする、**When** ページが表示される、**Then** メインカラーに淡青（#0B8BDB）が使用され、グラデーションのないフラットな背景が表示される
2. **Given** 練習ページ（PracticePage）にアクセスする、**When** プログレスバーやボタンを確認する、**Then** 淡青を基調としたデザインで統一されている
3. **Given** コードを確認する、**When** `tailwind.config.js` を見る、**Then** `karuta-tansei` カラーが定義されている
...
#### Design Requirements
- **FR-022**: アプリケーションは「淡青（Academic Blue）」スタイルを採用し、プライマリカラーとして `#0B8BDB` (Tansei Blue) を使用しなければならない
- **FR-023**: UIは「フラットデザイン」原則に従い、背景やカードにグラデーションを使用してはならない (No Gradients)
- **FR-024**: ボタンやアクセントカラーには、淡青と調和する補色（Gold/Orange `#F4A000`）を使用しなければならない
- **FR-025**: テキストは高い可読性を確保するため、ダークグレー（`#212121`）を基本としなければならない
...
- **SC-011**: トップページおよび練習ページの主要要素（ボタン、ヘッダー背景）が淡青（#0B8BDB）で表示されている
- **SC-012**: すべてのUIコンポーネントにおいて、意図しないグラデーション使用がなく、フラットなデザインが適用されている
- **SC-013**: `index.css` および `tailwind.config.js` が、新しいデザイン仕様（Academic Style）と整合している

...
function getStripe(): Stripe {
  if (!_stripe) {
    ...
    if (!key) throw new functions.https.HttpsError('internal', 'Stripe secret key not configured');
 ...
 * 課金レコード初期化（Callable）
 */
export const ensureBillingOnJoin = functions
...
export const createCheckoutSession = functions
  .region('asia-northeast1')
  .https.onCall(async (data: { successUrl?: string; cancelUrl?: string }, context) => {
    const uid = requireAuth(context);
    ...
    if (!priceId) throw new functions.https.HttpsError('internal', 'Stripe price ID not configured');
 ...
 * 内弟子割の設定（管理者のみ）
 */
export const setUchideshiFree = functions
  .region('asia-northeast1')
  .https.onCall(async (data: { uid: string; isUchideshiFree: boolean }, context) => {
 ...
 * 団体作成上限の設定（管理者のみ）
 */
export const adminSetMaxGroups = functions
  .region('asia-northeast1')
  .https.onCall(async (data: { uid: string; maxGroups: number }, context) => {
    await requireAdmin(context);
 ...
 */
export const adminGetUserBillingStatuses = functions
  .region('asia-northeast1')
  .https.onCall(async (data: { uids: string[] }, context) => {
    await requireAdmin(context);
...
// Internal export for use by other functions
export { ensureBillingOnJoinInternal };

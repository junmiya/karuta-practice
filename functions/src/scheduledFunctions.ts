 ...
 */
export const dailyRankingSnapshot = functions
  ...
  .onRun(async () => {
    ...
    try {
      ...
      const dailyScores: Map<
        string,
        Array<{ uid: string; nickname: string; score: number }>
      > = new Map();
 ...
 */
export const updateSeasonStatus = functions
  .region('asia-northeast1')
  .pubsub.schedule('1 0 * * *')
  .timeZone('Asia/Tokyo')
  .onRun(async () => {
    ...
    try {
      ...
      for (const doc of seasonsSnapshot.docs) {
        ...
        if (season.status !== newStatus) {
          ...
          console.log(
            `  Season ${season.seasonId}: ${season.status} -> ${newStatus}`
          );
          updatedCount++;
        }
      }
 ...
 */
export const expireStaleSession = functions
  .region('asia-northeast1')

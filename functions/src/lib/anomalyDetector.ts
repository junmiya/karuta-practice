...
export interface AnomalyResult {
  isValid: boolean;
  reasons: string[];
  reasonCodes: InvalidReasonCode[];
  ruleVersion: string;
}

export type InvalidReasonCode =
  | 'ROUND_COUNT_MISMATCH'      // round数不一致
  | 'ROUND_DUPLICATE'           // ラウンド重複
  | 'CHOICE_INTEGRITY_NG'       // 選択肢整合性NG
  | 'EXTREME_FAST'              // 極端な高速
  | 'EXTREME_SLOW'              // 極端な低速
  | 'CORRECT_COUNT_OUT_OF_RANGE' // 正答数範囲外
  | 'DISTRIBUTION_ANOMALY'      // 分布逸脱
  | 'TOTAL_TIME_TOO_SHORT'      // 合計時間が短すぎる
  | 'AVG_TIME_TOO_SHORT';       // 平均時間が短すぎる
...
export interface AnomalyConfig {
  ...
  // 時間閾値（ms）
  minResponseMs: number;          // 極端な高速判定閾値
  maxResponseMs: number;          // 極端な低速判定閾値
  minTotalMs: number;             // 合計時間の最低値
  minAvgMs: number;               // 平均時間の最低値

  // 回数閾値
  maxFastResponses: number;       // 極端な高速の許容回数
  maxSlowResponses: number;       // 極端な低速の許容回数

  // 分布異常
  distributionCheckEnabled: boolean;
  distributionFastThresholdMs: number;  // 分布異常判定用高速閾値
  distributionFastMaxRatio: number;     // 分布異常判定用高速比率（0-1）
}
...
// 段階0互換の旧設定
export const LEGACY_CONFIG: AnomalyConfig = {
...
export function detectAnomalies(
  ...
  const reasonCodes: InvalidReasonCode[] = [];
 ...
 * 詳細異常値検出（段階1）
 */
export function detectAnomaliesV2(
  rounds: Round[],
  correctCount: number,
  ...
  // 4. 極端な高速チェック
  const fastResponses = rounds.filter(
    (r) => r.clientElapsedMs < config.minResponseMs
  );
  if (fastResponses.length > config.maxFastResponses) {
    reasons.push(
      `極端に速い回答が多すぎます（${fastResponses.length}回、${config.minResponseMs}ms未満）`
    );
    reasonCodes.push('EXTREME_FAST');
  }

  // 5. 極端な低速チェック
  const slowResponses = rounds.filter(
    (r) => r.clientElapsedMs > config.maxResponseMs
  );
  if (slowResponses.length > config.maxSlowResponses) {
    reasons.push(
      `極端に遅い回答があります（${slowResponses.length}回、${config.maxResponseMs}ms超過）`
    );
  ...
  if (correctCount < 0 || correctCount > config.expectedRoundCount) {
    reasons.push(
      `正答数が範囲外です（${correctCount}、0-${config.expectedRoundCount}の範囲外）`
    );
    reasonCodes.push('CORRECT_COUNT_OUT_OF_RANGE');
  }
  ...
  if (totalElapsedMs < config.minTotalMs) {
    reasons.push(
      `合計回答時間が短すぎます（${totalElapsedMs}ms、最低${config.minTotalMs}ms）`
    );
    reasonCodes.push('TOTAL_TIME_TOO_SHORT');
  }
  ...
  if (config.distributionCheckEnabled) {
    const veryFastResponses = rounds.filter(
      (r) => r.clientElapsedMs < config.distributionFastThresholdMs
    );
    const fastRatio = veryFastResponses.length / rounds.length;
    if (fastRatio > config.distributionFastMaxRatio) {
      reasons.push(
        `回答時間の分布が異常です（${Math.round(fastRatio * 100)}%が${config.distributionFastThresholdMs}ms未満）`
      );
 ...
 */
export function getAnomalyConfig(isOfficial: boolean = true): AnomalyConfig {
  // 将来的にはFirestoreから動的に読み込む
  return isOfficial ? DEFAULT_CONFIG : LEGACY_CONFIG;
}

/**
 * 無効理由コードから人間可読なメッセージを取得
 */
export function getReasonMessage(code: InvalidReasonCode): string {
  const messages: Record<InvalidReasonCode, string> = {
